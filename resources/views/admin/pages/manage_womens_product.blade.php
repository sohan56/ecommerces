                         ...
                         <div class="widget-body">
                             <div>
                                 ...
                                 <tbody role="alert" aria-live="polite" aria-relevant="all">
                                    @foreach($all_wproduct as $v_wproduct)
                                    <tr class="odd ">
                                         <td class="  sorting_1">{{$v_wproduct->wproduct_id}}</td>
                                         <td class=" ">{{$v_wproduct->wproduct_name}}</td>
                                         <td class=" ">
                                             @if($v_wproduct->publication_status==1)
                                             Publish
                                             @else
                                             Unpublish
                                             @endif
                                         ...
                                         <td class="center ">
                                            @if($v_wproduct->publication_status==1)
                                            <a href="{{URL::to('/unpublish-womens-product',$v_wproduct->wproduct_id)}}" class="btn btn-danger"><i class="icon-thumbs-down"></i></a>
                                            @else
                                            <a href="{{URL::to('/publish-womens-product',$v_wproduct->wproduct_id)}}" class="btn btn-success"><i class="icon-thumbs-up"></i></a>
                                            @endif
                                            <a href="{{URL::to('/edit-womens-product',$v_wproduct->wproduct_id)}}" class="btn btn-primary"><i class="icon-pencil"></i></a>
                                            @if(Session::get('access_label')==2)
                                            <a href="{{URL::to('/delete-womens-prodict',$v_wproduct->wproduct_id)}}" class="btn btn-danger" onclick="return checkDelete()"><i class="icon-trash"></i></a>
                                            @endif
