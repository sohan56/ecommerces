                         ...
                         <div class="widget-body">
                             <div>
                                 ...
                                 <tbody role="alert" aria-live="polite" aria-relevant="all">
                                    @foreach($all_wcategory as $v_wcategory)
                                    <tr class="odd ">
                                         <td class="  sorting_1">{{$v_wcategory->wcategory_id}}</td>
                                         <td class=" ">{{$v_wcategory->wcategory_name}}</td>
                                         <td class=" ">
                                             @if($v_wcategory->publication_status==1)
                                             Publish
                                             @else
                                             Unpublish
                                             @endif
                                         ...
                                         <td class="center ">
                                            @if($v_wcategory->publication_status==1)
                                            <a href="{{URL::to('/unpublish-womens-category',$v_wcategory->wcategory_id)}}" class="btn btn-danger"><i class="icon-thumbs-down"></i></a>
                                            @else
                                            <a href="{{URL::to('/publish-womens-category',$v_wcategory->wcategory_id)}}" class="btn btn-success"><i class="icon-thumbs-up"></i></a>
                                            @endif
                                            <a href="{{URL::to('/edit-womens-category',$v_wcategory->wcategory_id)}}" class="btn btn-primary"><i class="icon-pencil"></i></a>
                                            @if(Session::get('access_label')==2)
                                            <a href="{{URL::to('/delete-womens-category',$v_wcategory->wcategory_id)}}" class="btn btn-danger" onclick="return checkDelete()"><i class="icon-trash"></i></a>
                                            @endif
